...
var (
	...
	levelMetricsTableBottomDivider       = strings.Repeat("-", levelMetricsTable.Width())
	...
	levelCompactionMetricsTableBottomDivider = strings.Repeat("-", compactionLevelMetricsTable.Width())
)
...
func (m *Metrics) String() string {
	wb := whiteboard.Make(92, levelMetricsTable.Width())
	var total LevelMetrics
	...
	renderLevelMetrics := levelMetricsTable.RenderFunc(pos)
	for l := 0; l < numLevels; l++ {
	...
	renderCompactionLevelMetrics := compactionLevelMetricsTable.RenderFunc(pos)
	for l := 0; l < numLevels; l++ {
		renderCompactionLevelMetrics(&m.Levels[l])
	}
	pos = renderCompactionLevelMetrics(&total)
	pos.Offset(-1, 0).WriteString("total")
	pos = pos.WriteString(levelCompactionMetricsTableBottomDivider).NewlineReturn()
