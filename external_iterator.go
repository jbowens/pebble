...
func createExternalPointIter(ctx context.Context, it *Iterator) (internalIterator, error) {
	...
	for _, readers := range it.externalReaders {
		...
		for _, r := range readers {
			...
			mlevels = append(mlevels, makeMergingIterLevel(&it.comparer, pointIter, rangeDelIter))
		}
		if len(combinedIters) == 1 {
			mlevels = append(mlevels, mergingIterLevel{iter: combinedIters[0]})
			...
			sli.init(it.opts)
			mlevels = append(mlevels, mergingIterLevel{iter: sli})
		}
	}
