...
// If start and end are specified, filter out any range spans that
...
func Truncate(
	...
	for key, value := iter.First(); key != nil; key, value = iter.Next() {
		...
		// The comparison between s.End and start is by user key only,
		// as the span is exclusive at s.End, so comparing by user keys
		// is sufficient. Alternatively, the below comparison can be
		// seen to be logically equivalent to:
