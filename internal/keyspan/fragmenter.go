...
// but that isn't true for range tombstones in the legacy range-del-v1
// block format.
...
// Fragmenter fragments a set of range spans such that overlapping
// spans are split at their overlap points. The fragmented spans are
...
type Fragmenter struct {
	...
	Emit func([]Span)
	// pending contains the list of pending key span fragments that have
	// not been flushed to the block writer. Note that the spans have not
	// been fragmented on the end keys yet. That happens as the spans are
...
func (f *Fragmenter) Covers(key base.InternalKey, snapshot uint64) bool {
	...
	for _, t := range f.pending {
		if f.Cmp(key.UserKey, t.End) < 0 {
			// NB: A range deletion tombstone does not delete a point
			// operation at the same sequence number.
...
// force emitting of spans which straddle an sstable boundary. Note that
// the emitted spans are not truncated to the specified key. Consider the
// scenario:
//
//     a---------k#10
//          f#8
...
// truncate the span [a,k) at f. Doing so could produce an sstable with
// the records:
//
//     a----f#10
//          f#8
...
func (f *Fragmenter) FlushTo(key []byte) {
	...
	// At this point we know that the new start key is greater than the
	// pending spans start keys. We flush all span fragments with a
	// start key <= key.
...
// spans to the specified end key by calling truncateAndFlush. Only called
...
// Start returns the start key of the first span in the pending buffer,
// or nil if there are no pending spans. The start key of all pending
// spans is the same as that of the first one.
	...
	for _, s := range pending {
		if f.Cmp(key, s.End) < 0 {
			...
			if f.Cmp(s.Start.UserKey, key) < 0 {
				done = append(done, Span{Start: s.Start, End: key})
			}
			f.pending = append(f.pending, Span{
				Start: base.MakeInternalKey(key, s.Start.SeqNum(), s.Start.Kind()),
				End:   s.End,
...
// flush a group of key spans to the block. The spans are required
// to all have the same start key. We flush all span fragments until
// startKey > lastKey. If lastKey is nil, all span fragments are
// flushed. The specification of a non-nil lastKey occurs during compaction
// where we want to flush (but not truncate) all spans that start at or
...
// contain the spans [a,e)#10 and [a,e)#9. Fragmentation would naturally
// produce a span [e,h)#9, but we don't need to output that span to
// the first sstable.
func (f *Fragmenter) flush(buf []Span, lastKey []byte) {
	...
	// Sort the spans by end key. This will allow us to walk over the
	// spans and easily determine the next split point (the smallest
	// end-key).
	f.sortBuf.cmp = f.Cmp
	f.sortBuf.buf = buf
	sort.Sort(&f.sortBuf)

	// Loop over the key spans, splitting by end key.
	for len(buf) > 0 {
		// A prefix of key spans will end at split. remove represents the
