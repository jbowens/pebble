...
// findFunc returns the index where the given item should be inserted into this
// list. 'found' is true if the item already exists in the list at the given
// index.
func (n *node) findFunc(f func(*FileMetadata) int) (index int, found bool) {
	// Logic copied from sort.Search. Inlining this gave
	// an 11% speedup on BenchmarkBTreeDeleteInsert.
	i, j := 0, int(n.count)
	for i < j {
...
func (s iterStack) clone() iterStack {
	// If the iterator is using the iterStackArr, we only need to copy the
	// struct itself.
	if s.s == nil {
		return s
	}
	clone := s
	clone.s = make([]iterFrame, len(s.s))
	copy(clone.s, s.s)
	return clone
}
...
func (i *iterator) Seek(cmp func(*FileMetadata) int) {
	i.reset()
	if i.n == nil {
		return
	}
	for {
		pos, found := i.n.findFunc(cmp)
		i.pos = int16(pos)
		if found {
			return
		}
