...
func NewParser() *dsl.Parser[Probe] {
	...
	probeParser.DefineFunc("If",
		func(p *dsl.Parser[Probe], s *dsl.Scanner) Probe {
			pred, err := predicateParser.ParseFromPos(s, s.Scan())
			if err != nil {
				panic(err)
			}
			thenProbe, err := probeParser.ParseFromPos(s, s.Scan())
			if err != nil {
				panic(err)
			}
			elseProbe, err := probeParser.ParseFromPos(s, s.Scan())
			if err != nil {
				panic(err)
			}
			s.Consume(token.RPAREN)
			return If(pred, thenProbe, elseProbe)
		})
...
func (p UserKey) Evaluate(pctx *ProbeContext) bool {
	return pctx.Op.Key != nil && pctx.Comparer.Equal(pctx.Op.Key.UserKey, p)
}
...
func (kv *returnKV) Probe(pctx *ProbeContext) {
	pctx.Op.Key = kv.InternalKey
	pctx.Op.Value = base.MakeInPlaceValue(kv.Value)
}
