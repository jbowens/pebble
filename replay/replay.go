...
func (r *Runner) prepareWorkloadSteps(ctx context.Context) error {
	...
	r.Opts.MaxOpenFiles = 10
	provider, closeFunc, err := pebble.SetupBlobReaderProvider(r.WorkloadFS, r.WorkloadPath, r.Opts, r.readerOpts)
	if err != nil {
		return err
	}
	defer closeFunc()
			...
			for {
				...
				case flushStepKind:
					...
					err := loadFlushedSSTableKeys(s.flushBatch, r.WorkloadFS, r.WorkloadPath,
						newFiles, blobRefMap, blobFileMap.Lookup, provider, r.readerOpts, &flushBufs)
					if err != nil {
...
func loadFlushedSSTableKeys(
	...
	blobFileLookup blob.FileLookupFunc,
	...
	for _, fileNum := range fileNums {
		if err := func() error {
			...
			vf, blobContext := sstable.LoadValBlobContext(blobFileLookup, provider, blobRefs)
