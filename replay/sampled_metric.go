...
// metric over time, per-second. The provided width and height determe the size
// of the graph and the number of representable discrete x and y points. All
// deltas are multiplied by the provided scale parameter and scaled to
...
func (m *SampledMetric) values(buckets int) (bucketDur time.Duration, values []float64) {
	...
	for i, b := 0, 0; i < len(m.samples); i++ {
		...
		bi := int(m.samples[i].since) / int(bucketDur)
		if bi == buckets {
			bi = buckets - 1
		}
		if b < bi {
...
func (m *SampledMetric) ValuesDelta(buckets int) []float64 {
	if len(m.samples) == 0 || buckets < 1 {
		return nil
	}
