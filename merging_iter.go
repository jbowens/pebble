...
func (m *mergingIter) isNextEntryDeleted(item *mergingIterLevel) (bool, error) {
	...
	for level := 0; level <= item.index; level++ {
		...
		if l.getTombstone() == nil {
			// If l.getTombstone returns nil, this level does not contain a
			// tombstone overlapping the current key.
			continue
		}
		if level < item.index {
		...
		if l.getTombstone().CoversAt(m.snapshot, item.iterKV.SeqNum()) {
