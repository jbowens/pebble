...
type levelIter struct {
	...
	shouldInterleaveRangeDels bool
}
...
func (l *levelIter) init(
	...
	l.shouldInterleaveRangeDels = false
}

func (l *levelIter) interleaveRangeDels() {
	l.shouldInterleaveRangeDels = true
}
...
func (l *levelIter) loadFile(file *fileMetadata, dir int) loadFileReturnIndicator {
	...
	for {
		...
		if rangeDelIter != nil {
			if l.shouldInterleaveRangeDels {
				l.interleaving.Init(l.comparer, iter, rangeDelIter, keyspan.InterleavingIterOpts{
...
func (l *levelIter) verify(key *InternalKey, val base.LazyValue) (*InternalKey, base.LazyValue) {
	...
	if invariants.Enabled && !l.disableInvariants && key != nil {
		...
		// the Iterator.
		if l.lower != nil && key.Kind() != base.InternalKeyKindRangeDelete && l.cmp(key.UserKey, l.lower) < 0 {
			l.logger.Fatalf("levelIter %s: lower bound violation: %s < %s\n%s", l.level, key, l.lower, debug.Stack())
		}
		if l.upper != nil && key.Kind() != base.InternalKeyKindRangeDelete && l.cmp(key.UserKey, l.upper) > 0 {
...
func (l *levelIter) SeekPrefixGE(
	...
	// table as findFileGE found the table where key <= meta.Largest.
	//
...
func (l *levelIter) getRangeDel() *keyspan.Span {
	if !l.shouldInterleaveRangeDels {
		return nil
	}
...
func (l *levelIter) skipEmptyFileForward() (*InternalKey, base.LazyValue) {
	...
	// presence of an upper-bound or the use of SeekPrefixGE.
	//
	...
	for ; key == nil; key, val = l.iter.First() {
		// We've exhausted the current sstable. If an upper bound is present
		// and the upper bound lies within the current sstable, then we will
		// have reached the upper bound rather than the end of the sstable.
		if l.tableOpts.UpperBound != nil {
			// This helps with performance when many levels are populated
			// with sstables and most don't have any actual keys within the
			// bounds.
			return nil, base.LazyValue{}
		}
...
func (l *levelIter) skipEmptyFileBackward() (*InternalKey, base.LazyValue) {
	...
	for ; key == nil; key, val = l.iter.Last() {
		// We've exhausted the current sstable. If a lower bound is present
		// and the lower bound lies within the current sstable, then we will
		// have reached the lower bound rather than the beginning of the
		// sstable.
		if l.tableOpts.LowerBound != nil {
			// This helps with performance when many levels are populated
			// with sstables and most don't have any actual keys within the
