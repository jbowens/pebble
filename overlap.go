...
// largestPossibleOverlappingSeqNum returns a sequence number n such that n is ≥
// the sequence number of every key in the LSM version that falls within
// [bounds].
func (c *overlapChecker) largestPossibleOverlappingSeqNum(
	...
	// oldest keys.
	for subLevel := 0; subLevel < len(c.readState.current.L0SublevelFiles); subLevel++ {
		manifestIter := c.readState.current.L0Sublevels.Levels[subLevel].Iter()
		pointOverlap, err := c.determinePointKeyOverlapInLevel(
			ctx, bounds, manifest.Level(0), manifestIter)
		if err != nil {
			return 0, err
		} else if pointOverlap {
			return largestSeqNumInBounds(c.comparer.Compare, manifestIter, bounds), nil
		}
	...
	// Check for overlap over the keys of L1..L6.
	for level := 1; level < numLevels; level++ {
		...
		// c.readState.current.RangeKeyLevels which only contains file metadata
		// for files that contain range keys. If a level contains both point and
		// range key overlap, the sequence number returned by
		// largestSeqNumInBounds using rangeManifestIter is not guaranteed to be
		// at least as high as the largest overlapping point key. Since the
		// check for point keys uses c.readState.current.Levels which contains
		// all files, the inverse is not true.

		pointManifestIter := c.readState.current.Levels[level].Iter()
		pointOverlap, err := c.determinePointKeyOverlapInLevel(
			ctx, bounds, manifest.Level(level), pointManifestIter)
		if err != nil {
			return 0, err
		} else if pointOverlap {
			return largestSeqNumInBounds(c.comparer.Compare, pointManifestIter, bounds), nil
		}

		rangeManifestIter := c.readState.current.RangeKeyLevels[level].Iter()
		rangeOverlap, err := c.determineRangeKeyOverlapInLevel(
			ctx, bounds, manifest.Level(level), rangeManifestIter)
		if err != nil {
			return 0, err
		} else if rangeOverlap {
			return largestSeqNumInBounds(c.comparer.Compare, rangeManifestIter, bounds), nil
		}
	}

	// There's no data overlap in L0...L6. We return a sequence number 1 less
	// than the oldest memtable's logSeqNum. A memtable's logSeqNum is
	// guaranteed to be ≤ all keys in the memtable. Since we don't know if any
	// keys in the memtable overlap with bounds and a key in the memtable may
	// have `logSeqNum` as its sequence number, we can't return logSeqNum
	// itself.
	return c.readState.memtables[0].logSeqNum - 1, nil
}

func largestSeqNumInBounds(
	cmp base.Compare, manifestIter manifest.LevelIterator, bounds base.UserKeyBounds,
) (largestSeqNum uint64) {
	f := manifestIter.SeekGE(cmp, bounds.Start)
	for f != nil && bounds.End.IsUpperBoundFor(cmp, f.Smallest.UserKey) {
		largestSeqNum = max(largestSeqNum, f.LargestSeqNum)
		f = manifestIter.Next()
	}
