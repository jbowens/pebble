...
func finishInitializingIter(buf *iterAlloc) *Iterator {
	...
	if dbi.opts.rangeKeys() {
		// When iterating over both point and range keys, avoid creating the
		...
		var useLazyCombinedIteration bool
		if dbi.rangeKey == nil && dbi.opts.KeyTypes == IterKeyTypePointsAndRanges &&
		...
		if useLazyCombinedIteration {
			...
			// Wrap an empty point iterator with an interleaving iterator that
			// interleaves range keys pulled from dbi.rangeKey.rangeKeyIter.
			//
		...
		// !dbi.opts.rangeKeys()
		//
		// Reset the combined iterator state.
...
func (i *Iterator) constructPointIter(
	memtables flushableList, buf *iterAlloc,
) {
	if i.pointIter != nil {
		// Already have one.
		return
