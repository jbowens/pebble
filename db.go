...
func (d *DB) newIterInternal(batch *Batch, s *Snapshot, o *IterOptions) *Iterator {
	...
	*dbi = Iterator{
		...
		newIters:            d.newIters,
		seqNum:              seqNum,
		newRangeKeyIter: func(dbi *Iterator) keyspan.FragmentIterator {
			return d.newRangeKeyIter(dbi, seqNum, batch, readState)
		},
	}
	if o != nil {
		dbi.opts = *o
...
func finishInitializingIter(buf *iterAlloc) *Iterator {
	...
	if dbi.opts.rangeKeys() {
		if dbi.rangeKey == nil {
			...
			dbi.rangeKey.rangeKeyIter = dbi.newRangeKeyIter(dbi)
		}
