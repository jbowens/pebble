...
func newPickedCompactionFromL0(lcf *manifest.L0CompactionFiles, opts *Options, vers *version, baseLevel int, isBase bool) *pickedCompaction {
	...
	pc.startLevel.files = make([]*manifest.FileMetadata, 0, len(lcf.Files))
	for j := range lcf.FilesIncluded {
		if lcf.FilesIncluded[j] {
			pc.startLevel.files = append(pc.startLevel.files, vers.Levels[0][j])
		}
	}
	return pc
}
...
func (pc *pickedCompaction) expandInputs(level int, inputs []*fileMetadata) []*fileMetadata {
	if level == 0 {
	...
	for ; start > 0; start-- {
		cur := files[start]
		prev := files[start-1]
		if pc.cmp(prev.Largest.UserKey, cur.Smallest.UserKey) < 0 {
			break
		}
	...
	for ; end < len(files); end++ {
		cur := files[end-1]
		next := files[end]
		if pc.cmp(cur.Largest.UserKey, next.Smallest.UserKey) < 0 {
			break
		}
...
type candidateLevelInfo struct {
	...
	// compaction, and a pickedCompaction created for the compaction.
	file int
}
...
func pickAutoHelper(
	...
	pc.startLevel.files = vers.Levels[pc.startLevel.level][cInfo.file : cInfo.file+1]
