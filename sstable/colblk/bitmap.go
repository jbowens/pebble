...
func (b nullBitmapBuilder) Size(offset uint32) uint32 {
	...
	offset += uint32(len(b)) * nullBitmapAlign
	return offset
}

func nullBitmapSize(offset uint32, rows int) uint32 {
	return (align(offset, nullBitmapAlign) - offset) +
		uint32((rows+nullBitmapHalfWordSize-1)/nullBitmapHalfWordSize)*nullBitmapAlign
}
