...
func TestPrefixCompress(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/prefix_compress", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "compress":
			...
			i := b.prefixCompress(rows, len(b.offsets), buf)
			wrapStr(&out, string(buf[:i]), 80)
			return out.String()
		default:
...
func TestRawBytes(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/raw_bytes", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "build":
			...
			endOffset, _ := builder.Finish(0, count, uint32(startOffset), buf)
	...
	datadriven.RunTest(t, "testdata/prefix_bytes", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "put":
			...
			for _, k := range inputKeys {
				...
				p := []byte(k)
				builder.PutOrdered(p, keyPrefixLenSharedWithPrev)
				keys++
		...
		case "finish":
			...
			offsets := make([]uint32, len(builder.offsets))
			copy(offsets, builder.offsets)
			offset, _ := builder.Finish(0, rows, 0, buf)
			...
			pb = makePrefixBytes(uint32(rows), unsafe.Pointer(unsafe.SliceData(buf)))
...
func TestPrefixBytesRandomized(t *testing.T) {
	...
	for i := 0; i < len(userKeys); i++ {
		...
		bb.PutOrdered(userKeys[i], keyPrefixLenSharedWithPrev)
	}

	size := bb.Size(len(userKeys), 0)
	buf := make([]byte, size)
	offset, _ := bb.Finish(0, len(userKeys), 0, buf)
