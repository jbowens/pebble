...
// Rows returns the number of rows in the index block.
func (w *IndexBlockWriter) Rows() int { return w.rows }
...
// AddMetaBlockHandle adds a new BlockHandle entry. AddMetaBlockHandle should be
// used only when building a meta index block.
func (w *IndexBlockWriter) AddMetaBlockHandle(key string, bh block.Handle) int {
	idx := w.rows
	w.separators.Put(unsafe.Slice(unsafe.StringData(key), len(key)))
	w.endOffsets.Set(idx, bh.Offset-bh.Length-block.TrailerLen)
	// When building a meta index block, every entry should have a length. This
	// means the non-NULL index should always match the row index.
	b, j := w.lengths.NotNull(idx)
	if j != idx {
		panic("pebble: missing index block length")
	}
...
func (r *IndexReader) Init(data []byte) {
	...
	r.hasExplicitLens = lengths.NullBitmap.Empty() // all non-NULL
...
func (i *IndexIter) DataBlockHandle() (bh block.Handle) {
	if invariants.Enabled && i.r.hasExplicitLens {
...
// BlockHandle returns the index block handle at the iterator's current
// position. It must only be used for second-level index blocks or meta index
// blocks.
func (i *IndexIter) BlockHandle() (bh block.Handle) {
	if invariants.Enabled && !i.r.hasExplicitLens {
		panic(errors.AssertionFailedf("pebble: BlockHandle called on first-level index iterator"))
	}
