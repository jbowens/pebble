...
// DataBlockIterConfig wraps a DataBlockIter, implementing the block package's
// DataBlockIterator. DataBlockIterConfig retains configuration (the KeySchema
// and lazy value retriever) across inititalization for additional blocks. It
...
// reused, it does not retain any configuration beyond ba reset.
type DataBlockIterConfig struct {
	DataBlockIter
...
var _ block.DataBlockIterator = (*DataBlockIterConfig)(nil)

// InitHandle initializes the block from the provided buffer handle.
func (c *DataBlockIterConfig) InitHandle(
...
func (c *DataBlockIterConfig) Handle() block.BufferHandle {
	return c.h
}

// Invalidate invalidates the block iterator, removing references to the block
// it was initialized with.
func (c *DataBlockIterConfig) Invalidate() {
...
func (c *DataBlockIterConfig) IsDataInvalidated() bool {
	return c.DataBlockIter.r == nil
}

// Close implements the base.InternalIterator interface.
func (i *DataBlockIterConfig) Close() error {
	if i.keySeeker != nil {
		i.keySeeker.Release()
		i.keySeeker = nil
	}
