...
func DecodeColumn[V any](r *BlockReader, col int, dataType DataType, decodeFunc DecodeFunc[V]) V {
	...
	v, _ := decodeFunc(r.data, r.pageStart(col), int(r.header.Rows))
	return v
}
...
// column. The column must be of type DataTypePrefixBytes.
