...
func runBlockPropsCmd(r *Reader, td *datadriven.TestData) string {
	...
	i, err := rowblk.NewIter(r.Compare, r.Split, bh.Get(), block.NoTransforms, false /* hasValuePrefix */)
	if err != nil {
		return err.Error()
	}
	defer bh.Release()
	...
	for kv := i.First(); kv != nil; kv = i.Next() {
		...
		if twoLevelIndex {
			...
			if err := subiter.Init(r.Compare, r.Split, subIndex.Get(), block.NoTransforms, false /* hasValuePrefix */); err != nil {
				return err.Error()
			}
