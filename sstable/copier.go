...
func intersectingIndexEntries(
	...
	top, err := rowblk.NewIter(r.Compare, r.Split, indexH.Get(), NoTransforms, false /* usesValuePrefix */)
	if err != nil {
		return nil, err
	}
	defer top.Close()
	...
	for kv := top.SeekGE(start.UserKey, base.SeekGEFlagsNone); kv != nil; kv = top.Next() {
		...
		if r.Properties.IndexType != twoLevelIndex {
			...
			sub, err := rowblk.NewIter(r.Compare, r.Split, subBlk.Get(), NoTransforms, false /* usesValuePrefix */)
			if err != nil {
				return nil, err
			}
