...
func Alloc(n int, bufferPool *BufferPool) Value {
	if bufferPool != nil {
		return Value{buf: bufferPool.Alloc(n + TrailerLen)}
	}
	return Value{v: cache.Alloc(n + TrailerLen)}
}
...
func (b Value) Unpack() (Buf, *cache.Value) {
	return b.buf, b.v
}
