...
func check(fs vfs.FS, filename string, comparer *Comparer, fp FilterPolicy) error {
	...
	for k, v := range wordCount {
		...
		// Check using SeekGE.
		iter, err := r.NewIter(NoTransforms, nil /* lower */, nil /* upper */, base.NoBlobFetches)
		if err != nil {
			return err
		}
	...
	for i := 0; i < 10; i++ {
		...
		iter, err := r.NewIter(NoTransforms, lower, upper, base.NoBlobFetches)
		if err != nil {
			return err
		}
...
func TestReaderSymtheticSeqNum(t *testing.T) {
	...
	iter, err := r.NewIter(transforms, nil /* lower */, nil /* upper */, base.NoBlobFetches)
	require.NoError(t, err)
	for kv := iter.First(); kv != nil; kv = iter.Next() {
