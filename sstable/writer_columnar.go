...
func (w *ColumnarWriter) flushIndexBlock(sep []byte) block.Handle {
...
func (w *ColumnarWriter) Close() (err error) {
	...
	indexBH = w.flushIndexBlock(succ)
	...
	if indexPartitions := w.topLevelIndexBlock.Rows(); indexPartitions > 1 {
		...
		indexBH = block.Handle{Offset: w.size, Length: uint64(len(cb.compressed))}
		w.size += size
		w.writeQueue.ch <- cb
	}
	...
	if w.filterBlock != nil {
		b, err := w.filterBlock.finish()
		if err != nil {
			return err
		}
		cb := compressedBlockPool.Get().(*compressedBlock)
		cb.blockBuf.checksummer.Type = w.opts.Checksum
		cb.compressed, cb.trailer = compressAndChecksum(b, NoCompression, &cb.blockBuf)
		size := cb.size()

		w.props.FilterPolicyName = w.filterBlock.policyName()
		w.props.FilterSize = size
		meta.filter = block.Handle{Offset: w.size, Length: size}
		w.size += size
		w.writeQueue.ch <- cb
	}
	...
	if w.props.NumRangeDeletions > 0 {
		uncompressedRangeDelBlock := w.rangeDelBlock.Finish()
		w.rangeDelBlock.Reset()
		cb := compressedBlockPool.Get().(*compressedBlock)
		cb.blockBuf.checksummer.Type = w.opts.Checksum
		cb.compressed, cb.trailer = compressAndChecksum(uncompressedRangeDelBlock, NoCompression, &cb.blockBuf)
		size := cb.size()

		meta.rangeDeletions = block.Handle{Offset: w.size, Length: size}
		w.size += size
		w.writeQueue.ch <- cb
	}
	...
	if w.props.NumRangeKeys() > 0 {
		uncompressedRangeKeyBlock := w.rangeKeyBlock.Finish()
		w.rangeKeyBlock.Reset()
		cb := compressedBlockPool.Get().(*compressedBlock)
		cb.blockBuf.checksummer.Type = w.opts.Checksum
		cb.compressed, cb.trailer = compressAndChecksum(uncompressedRangeKeyBlock, NoCompression, &cb.blockBuf)
		size := cb.size()

		meta.rangeKeys = block.Handle{Offset: w.size, Length: size}
		w.size += size
		w.writeQueue.ch <- cb
	}
	...
	*/
	...
	metaIndexBH := w.writeMetaIndex(meta)
	...
	footer := footer{
		format:      w.opts.TableFormat,
		checksum:    w.opts.Checksum,
...
type metaIndex struct {
	filter         block.Handle
	rangeDeletions block.Handle
	rangeKeys      block.Handle
	valueIndex     block.Handle
	properties     block.Handle
}

func (w *ColumnarWriter) writeMetaIndex(idx metaIndex) block.Handle {
	// Write out the block handles sorted by their keys.
	if idx.filter.Length > 0 {
		w.metaIndexBlock.AddMetaBlockHandle(w.filterBlock.metaName(), idx.filter)
	}
	w.metaIndexBlock.AddMetaBlockHandle(metaPropertiesName, idx.properties)
	if idx.rangeDeletions.Length > 0 {
		w.metaIndexBlock.AddMetaBlockHandle(metaRangeDelV2Name, idx.rangeDeletions)
	}
	if idx.rangeKeys.Length > 0 {
		w.metaIndexBlock.AddMetaBlockHandle(metaRangeKeyName, idx.rangeKeys)
	}
	if idx.valueIndex.Length > 0 {
		w.metaIndexBlock.AddMetaBlockHandle(metaValueIndexName, idx.valueIndex)
	}

	// Write out the block.
	uncompressedMetaIndexBlock := w.metaIndexBlock.Finish()
	w.metaIndexBlock.Reset()
	cb := compressedBlockPool.Get().(*compressedBlock)
	cb.blockBuf.checksummer.Type = w.opts.Checksum
	cb.compressed, cb.trailer = compressAndChecksum(uncompressedMetaIndexBlock, NoCompression, &cb.blockBuf)
	size := cb.size()

	bh := block.Handle{Offset: w.size, Length: size}
	w.size += size
	w.writeQueue.ch <- cb
	return bh
}
