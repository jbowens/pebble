...
// A ValueFetcher retrieves values stored out-of-band in a separate blob file.
...
// Fetch returns the value, given the handle.
...
func (r *ValueFetcher) retrieve(ctx context.Context, vh Handle) (val []byte, err error) {
	...
	if cr == nil {
		...
		if cr.r, cr.closeFunc, err = r.readerProvider.GetValueReader(ctx, cr, vh.FileNum); err != nil {
			return nil, err
		}
...
// Close closes the ValueFetcher and releases all cached readers.
