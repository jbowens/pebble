...
func (s *sstableT) runCheck(cmd *cobra.Command, args []string) {
	...
	s.foreachSstable(stderr, args, func(path string, r *sstable.Reader) {
		...
		// files uncovered by scanning the directory entries.

		iter, err := r.NewIter(sstable.NoTransforms, nil, nil, base.BlobFetcherTODO)
		if err != nil {
		...
		prefixIter, err := r.NewIter(sstable.NoTransforms, nil, nil, base.BlobFetcherTODO)
		if err != nil {
			fmt.Fprintf(stderr, "%s\n", err)
			return
		}
...
func (s *sstableT) runScan(cmd *cobra.Command, args []string) {
	...
	s.foreachSstable(stderr, args, func(path string, r *sstable.Reader) {
		...
		// blob value handles, and one that fetches the blob values from blob
		// files uncovered by scanning the directory entries.
		iter, err := r.NewIter(sstable.NoTransforms, nil, s.end, base.BlobFetcherTODO)
		if err != nil {
